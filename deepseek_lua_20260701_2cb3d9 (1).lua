...
menu.BackgroundTransparency = 0.1 -- Сделаем менее прозрачным, чтобы было видно
...
local function toggleMenu()
    ...
    -- Если меню открыто, выведем его позицию для отладки
    if menu.Visible then
        print("Позиция меню:", menu.Position)
        print("Размер меню:", menu.Size)
    end
end
...
-- ==================== ПРИНУДИТЕЛЬНО ПОКАЗАТЬ МЕНЮ ЧЕРЕЗ 3 СЕКУНДЫ (для проверки) ====================
task.wait(3)
menu.Visible = true
print("Меню принудительно открыто через 3 секунды")
