-- ============================================
-- @set9p | SILENT AIM + AUTO TRIGGER + ANTI-KICK + JUMP SPEED + WALLSHOT + ESP + BHOP
-- ============================================
...
local Settings = {
    ...
    -- Auto Trigger & Visible Check & WallShot
    AutoTrigger = false,
    VisibleCheck = true,
...
local function isVisible(targetPart)
    if Settings.WallShot then return true end -- Принудительный обход проверок стен
    ...
    if Settings.WallShot and targetPart.Parent then
...
CombatTab:CreateToggle({
    Name = "Стрельба сквозь стены (WallShot)",
    CurrentValue = false,
    Flag = "WallShotToggle",
    Callback = function(v) Settings.WallShot = v end
})
...
Rayfield:Notify({
    Title = "@set9p",
    Content = "Крутилка починена, WallShot обновлен!",
    Duration = 3,
    Image = 4483362458,
})