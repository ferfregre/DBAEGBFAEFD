...
local Settings = {
    ...
    MaxWallThickness = 4,      -- Уменьшено до 4 студов для надежности
    MaxTargetWorldDistance = 150, -- Абсолютный лимит дальности до цели (чтобы не стреляло со спавна)
...
-- Строгая проверка простреливаемости (бетон и тяжелые материалы блокируются)
local function isPenetrable(hitPart)
    if not hitPart then return false end
    
    -- Блокируем железобетон, кирпич, гранит, скалы и неразрушаемые покрытия
    local mat = hitPart.Material
    if mat == Enum.Material.Concrete or 
       mat == Enum.Material.Brick or 
       mat == Enum.Material.Granite or 
       mat == Enum.Material.Slate or 
       mat == Enum.Material.CorrodedMetal or 
       mat == Enum.Material.Metal then
        return false
    end
    ...
    -- Простреливаем только стекло, дерево, пластик небольшой плотности или прозрачные объекты
    if mat == Enum.Material.Glass or mat == Enum.Material.Wood or mat == Enum.Material.WoodPlanks or hitPart.Transparency > 0.05 then
        return true
    end
...
local function canPenetrateShoot(targetPart)
    ...
    local worldDist = (origin - targetPos).Magnitude
    if worldDist > Settings.MaxWallbangDistance or worldDist > Settings.MaxTargetWorldDistance then
        return false
    end
...
local function GetClosestPlayer()
    ...
    for _, v in pairs(plrs) do
        ...
        -- Жесткий отсеивач целей, которые находятся слишком далеко (например, со спавна)
...
Rayfield:Notify({
    Title = "@set9p",
    Content = "Скрипт обновлен: Бетон блокируется, спавн-лок активирован!",
    Duration = 3,
    Image = 4483362458,
})