...
local function GetClosestPlayer()
    ...
    for _, v in pairs(plrs) do
        ...
        if Settings.HitPath == "Head" or Settings.HitPart == "Head" then
...
-- Моментальная проверка прицела строго по хитбоксу врага (Head / Torso)
        ...
        if model then
            ...
            if plr and isEnemy(plr) then
                ...
                if char then
                    ...
                    -- Проверяем, что прицел наведен именно на ключевую часть тела
                    if hitPart == head or hitPart == torso or hitPart.Parent == head or hitPart.Parent == torso then
                        return true
                    end
                end
            end
...
RunService.RenderStepped:Connect(function()
    ...
    -- Моментальный спам-триггер без пауз и задержек
...
Rayfield:Notify({
    Title = "@set9p",
    Content = "Моментальный триггер без пауз успешно внедрен!",
    Duration = 3,
    Image = 4483362458,
})