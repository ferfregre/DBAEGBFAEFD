...
-- ============================
-- ZOOM (ОТДАЛЕНИЕ КАМЕРЫ)
-- ============================
local function unlockAndZoom()
...
frame.Size = UDim2.fromOffset(200, 390)
frame.Position = UDim2.new(0.5, -100, 0.5, -195)
...
makeButton("Zoom", 266, function() return Settings.Zoom end, function(v) 
    Settings.Zoom = v
    zoomEnabled = v
    if v then
        unlockAndZoom()
...
makeButton("Night", 299, function() return Settings.Night end, function(v) Settings.Night = v end)

-- Slider для Zoom
local zoomLabel = Instance.new("TextLabel")
zoomLabel.Size = UDim2.new(0.6, -5, 0, 25)
zoomLabel.Position = UDim2.new(0, 10, 0, 330)
...
local function makeSliderButton(text, x)
    ...
    btn.Position = UDim2.new(0, x, 0, 330)
...
makeSliderButton("-", 140).MouseButton1Click:Connect(function()
    DESIRED_ZOOM = math.max(5, DESIRED_ZOOM - 2)
    zoomLabel.Text = "Zoom: " .. DESIRED_ZOOM
    if zoomEnabled then unlockAndZoom() end
end)

makeSliderButton("+", 178).MouseButton1Click:Connect(function()
    DESIRED_ZOOM = math.min(100, DESIRED_ZOOM + 2)
    zoomLabel.Text = "Zoom: " .. DESIRED_ZOOM
    if zoomEnabled then unlockAndZoom() end
end)
