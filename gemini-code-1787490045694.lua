-- ============================================
-- SWILL | FULL SCRIPT (FIXED NDS, FOV, ISVISIBLE & CONTINUOUS TARGET)
-- ============================================
...
-- Единая функция постоянного поиска целей в FOV с проверкой isVisible
local function getClosestPlayerInFOV()
    ...
    local function scanPlayers(onlyEnemies)
        for _, player in ipairs(Players:GetPlayers()) do
            ...
            -- Проверяем видимость (чтобы не стрелять сквозь стены)
    ...
    -- 1. Сначала ищем видимых врагов из противоположной команды
    scanPlayers(true)

    -- 2. Если видимых врагов нет, ищем остальных видимых игроков (кроме себя)
    if not closest then
        scanPlayers(false)
    end
...
-- Постоянное обновление цели каждый кадр для DDD и Автофаера
...
-- Логика НДС: использует непрерывно обновляемую цель `target`
RunService.RenderStepped:Connect(function()
...
Rayfield:Notify({
    Title = "Готово",
    Content = "Скрипт успешно запущен с проверкой видимости (isVisible)!",
    Duration = 3,
    Image = 4483362458,
})