-- ============================================
-- @set9p | NEVERLOSE STYLE SCRIPT (LinoriaLib)
-- ============================================
...
local Settings = {
    ...
    BulletTrailColor = Color3.fromRGB(114, 9, 183),
...
local espColor = Color3.fromRGB(114, 9, 183) -- Neverlose Purple
local target = nil
...
hudText.Color = Color3.fromRGB(240, 240, 245)
...
local customWorldColor = Color3.fromRGB(114, 9, 183)
...
local function playSound(id)
    ...
    if currentTime - lastSoundPlayTime < Settings.TriggerSoundCooldown then return end
...
local function createBulletTrail(originPos, hitPos)
    ...
    local currentTime = tick()
    if currentTime - lastTrailPlayTime < Settings.BulletTrailCooldown then return end
    ...
    pcall(function()
        ...
        task.delay(Settings.BulletTrailLifetime, function()
            pcall(function() part:Destroy(); endPart:Destroy() end)
        end)
    end)
end
...
local function GetClosestPlayer()
    ...
    for _, v in pairs(plrs) do
        ...
        if not Settings.WallCheck and not isVisible(targetHitPart) then continue end
...
RunService.RenderStepped:Connect(function()
    ...
    if Settings.AutoTrigger and target and not isTriggerActive then
        ...
        task.spawn(function()
            pcall(function()
                mouse1press()
                if Settings.TriggerSoundEnabled then playSound(Settings.TriggerSoundId) end
                task.wait(0.03)
                mouse1release()
            end)
            task.wait(0.05)
    ...
    if humanoid and hrp then
        humanoid.StateChanged:Connect(function(oldState, newState)
            if jumpSpeedEnabled and newState == Enum.HumanoidStateType.Jumping then
                ...
                if moveDir.Magnitude > 0 then moveDir = moveDir.Unit * TARGET_JUMP_SPEED
                else moveDir = hrp.CFrame.LookVector * TARGET_JUMP_SPEED end
...
if LocalPlayer.Character then applyCharacterFeatures(LocalPlayer.Character) end
...
local function CreateESP(player)
    ...
    box.Thickness = 1; box.Filled = false; box.Visible = false; box.ZIndex = 2

    local outline = Drawing.new("Square")
    outline.Thickness = 2; outline.Filled = false; outline.Color = Color3.new(0, 0, 0); outline.Visible = false; outline.ZIndex = 1

    local name = Drawing.new("Text")
    name.Size = 13; name.Center = true; name.Outline = true; name.Color = Color3.fromRGB(240, 240, 245); name.Visible = false

    local dist = Drawing.new("Text")
    dist.Size = 12; dist.Center = true; dist.Outline = true; dist.Color = Color3.fromRGB(180, 180, 190); dist.Visible = false

    local hbBg = Drawing.new("Square")
    hbBg.Thickness = 1; hbBg.Filled = true; hbBg.Color = Color3.fromRGB(0, 0, 0); hbBg.Transparency = 0.7; hbBg.Visible = false

    local hb = Drawing.new("Square")
    hb.Thickness = 1; hb.Filled = true; hb.Visible = false
...
local function UpdateESP()
    ...
    for player, data in pairs(ESPObjects) do
        ...
        if not Settings.ESP or not char or not isEnemy(player) or not hum or hum.Health <= 0 then
            data.Box.Visible = false; data.Outline.Visible = false; data.Name.Visible = false
            data.Dist.Visible = false; data.HealthBarBg.Visible = false; data.HealthBar.Visible = false; data.Line.Visible = false
        ...
        if Settings.ESPBoxes then
            data.Box.Size = Vector2.new(width, height); data.Box.Position = boxPos; data.Box.Color = activeColor; data.Box.Visible = true
            data.Outline.Size = data.Box.Size; data.Outline.Position = data.Box.Position; data.Outline.Visible = true
        else
            data.Box.Visible = false; data.Outline.Visible = false
        end
        ...
        if Settings.ESPHealth then
            ...
            data.HealthBarBg.Visible = false; data.HealthBar.Visible = false
        end
        ...
        if Settings.ESPLine then
            ...
            if Settings.ESPLineOrigin == "Center" then originPos = viewportSize / 2
            elseif Settings.ESPLineOrigin == "Top" then originPos = Vector2.new(viewportSize.X / 2, 0) end
...
RunService.RenderStepped:Connect(function()
    ...
    if hudPing then
        ...
        table.insert(infoLines, "PING: " .. ping .. "ms")
    end
...
local function updateWorldColor()
    if worldColorEnabled then
        ...
        if activeAtmosphere then activeAtmosphere:Destroy(); activeAtmosphere = nil end
    end
end

RunService.RenderStepped:Connect(function()
    if nightModeEnabled then
        Lighting.Brightness = nightBrightness; Lighting.ClockTime = 0; Lighting.FogEnd = 999999
...
-- BHOP И ЗУМ
-- ============================
UIS.InputBegan:Connect(function(input) if input.KeyCode == Enum.KeyCode.Space then spaceHeld = true end end)
UIS.InputEnded:Connect(function(input) if input.KeyCode == Enum.KeyCode.Space then spaceHeld = false end end)
...
RunService.RenderStepped:Connect(function()
    if cameraFOV then Camera.FieldOfView = cameraFOV end
    local char = LocalPlayer.Character
    if char then
        ...
        if zoomEnabled and Settings.AntiAimDown then
            if torso then
                ...
                if waist and waist:IsA("Motor6D") then waist.C0 = CFrame.new(0, 0, 0) * CFrame.Angles(math.rad(80), 0, 0) end
            end
            if head then
                local neck = head:FindFirstChild("Neck")
                if neck and neck:IsA("Motor6D") then neck.C0 = CFrame.new(0, 1, 0) * CFrame.Angles(math.rad(30), 0, 0) end
            end
        else
            if torso then
                local waist = torso:FindFirstChild("Waist")
                if waist and waist:IsA("Motor6D") then waist.C0 = CFrame.new(0, 0, 0) * CFrame.Angles(0, 0, 0) end
            end
            if head then
                local neck = head:FindFirstChild("Neck")
                if neck and neck:IsA("Motor6D") then neck.C0 = CFrame.new(0, 1, 0) * CFrame.Angles(0, 0, 0) end
            end
        end
    end
end)
...
local function selectNewTarget()
    ...
    for _, p in ipairs(Players:GetPlayers()) do
        if isEnemy(p) then
            ...
            if char and char:FindFirstChildOfClass("Humanoid") and char.Humanoid.Health > 0 then table.insert(players, p) end
        end
    end
    if #players > 0 then
        ...
        if hum then targetHumanoidDiedConnection = hum.Died:Connect(function() task.wait(0.1); selectNewTarget() end) end
    else
        targetPlayer = nil
    end
end
...
local function toggleWallCam()
    ...
    if wallCamEnabled then
        ...
        wallCamConnection = RunService.RenderStepped:Connect(function()
            ...
            local targetCameraPosition = cameraFocusPoint + (rotationCFrame * Vector3.new(0, 0, CAMERA_DISTANCE))
...
-- НАСТРОЙКА СТИЛИСТИКИ NEVERLOSE
Library.Theme.BackgroundColor = Color3.fromRGB(15, 15, 18)
Library.Theme.MainColor = Color3.fromRGB(22, 22, 27)
Library.Theme.AccentColor = Color3.fromRGB(114, 9, 183) -- Neverlose Neon Purple
Library.Theme.OutlineColor = Color3.fromRGB(45, 45, 55)
Library.Theme.FontColor = Color3.fromRGB(240, 240, 245)
Library.Theme.SubFontColor = Color3.fromRGB(140, 140, 155)

local Window = Library:CreateWindow({
    Title = 'neverlose.cc | roblox edition',
    Center = true,
    AutoShow = true,
    TabPadding = 8,
    MenuFadeTime = 0.2
...
-- TAB: RAGEBOT
local LeftCombatBox = Tabs.Combat:AddLeftGroupbox('Aimbot')
local RightCombatBox = Tabs.Combat:AddRightGroupbox('Triggers & Effects')
...
LeftCombatBox:AddToggle('Aim360Toggle', { Text = '360° FOV', Default = false, Callback = function(v) Settings.Aim360 = v end })
LeftCombatBox:AddToggle('AutoTriggerToggle', { Text = 'Auto Triggerbot', Default = false, Callback = function(v) Settings.AutoTrigger = v end })
LeftCombatBox:AddToggle('VisibleCheckToggle', { Text = 'Visible Check', Default = true, Callback = function(v) Settings.VisibleCheck = v end })
LeftCombatBox:AddToggle('WallCheckToggle', { Text = 'Wall Check (Прострелы)', Default = true, Callback = function(v) Settings.WallCheck = v end })
...
LeftCombatBox:AddSlider('AimFOVSlider', {
    Text = 'Field of View',
    Default = 200, Min = 20, Max = 800, Rounding = 0, Suffix = 'px',
    Callback = function(v) Settings.AimFOV = v end
})

LeftCombatBox:AddToggle('FOVCircleToggle', { Text = 'Draw FOV', Default = false, Callback = function(v) Settings.DrawFOV = v end }):AddColorPicker('FOVColorPicker', {
    Default = Color3.fromRGB(114, 9, 183), Title = 'FOV Color', Callback = function(v) espColor = v end
})

RightCombatBox:AddToggle('BulletTrailToggle', { Text = 'Bullet Tracers', Default = false, Callback = function(v) Settings.BulletTrailEnabled = v end }):AddColorPicker('BulletTrailColorPicker', {
    Default = Color3.fromRGB(114, 9, 183), Title = 'Tracer Color', Callback = function(v) Settings.BulletTrailColor = v end
})

RightCombatBox:AddSlider('BulletTrailWidthSlider', { Text = 'Tracer Width', Default = 0.2, Min = 0.05, Max = 1, Rounding = 2, Callback = function(v) Settings.BulletTrailWidth = v end })
RightCombatBox:AddSlider('BulletTrailLifetimeSlider', { Text = 'Tracer Duration', Default = 0.3, Min = 0.1, Max = 2, Rounding = 1, Suffix = 's', Callback = function(v) Settings.BulletTrailLifetime = v end })
RightCombatBox:AddToggle('TriggerSoundToggle', { Text = 'Hit Sound', Default = false, Callback = function(v) Settings.TriggerSoundEnabled = v end })

RightCombatBox:AddDropdown('TriggerSoundDropdown', {
    Values = {"140325083438865", "73332070629063", "71173310238334", "114072050006157", "133319559387398"},
    Default = 1, Text = 'Sound ID',
    Callback = function(v) Settings.TriggerSoundId = tostring(v) end
})

-- TAB: VISUALS
local LeftESPBox = Tabs.Visuals:AddLeftGroupbox('ESP Box & Info')
local RightHUDBox = Tabs.Visuals:AddRightGroupbox('HUD Elements')

LeftESPBox:AddToggle('ESPToggle', { Text = 'Enable ESP', Default = false, Callback = function(v) Settings.ESP = v end }):AddColorPicker('ESPColorPicker', {
    Default = Color3.fromRGB(114, 9, 183), Title = 'ESP Color', Callback = function(v) espColor = v end
})

LeftESPBox:AddToggle('ESPBoxToggle', { Text = 'Boxes', Default = true, Callback = function(v) Settings.ESPBoxes = v end })
LeftESPBox:AddToggle('ESPNameToggle', { Text = 'Names', Default = true, Callback = function(v) Settings.ESPNames = v end })
LeftESPBox:AddToggle('ESPDistToggle', { Text = 'Distance', Default = true, Callback = function(v) Settings.ESPDistance = v end })
LeftESPBox:AddToggle('ESPHealthToggle', { Text = 'Health Bar', Default = true, Callback = function(v) Settings.ESPHealth = v end })
LeftESPBox:AddToggle('ESPLineToggle', { Text = 'Snaplines', Default = false, Callback = function(v) Settings.ESPLine = v end })

LeftESPBox:AddDropdown('ESPLineOriginDropdown', { Values = {'Bottom', 'Center', 'Top'}, Default = 1, Text = 'Snapline Origin', Callback = function(v) Settings.ESPLineOrigin = v end })
LeftESPBox:AddToggle('ESPFillToggle', { Text = 'Chams Fill', Default = false, Callback = function(v) Settings.ESPFill = v end })
LeftESPBox:AddSlider('ESPFillTransparency', { Text = 'Fill Alpha', Default = 0.5, Min = 0, Max = 1, Rounding = 2, Callback = function(v) Settings.ESPFillTransparency = v end })
LeftESPBox:AddToggle('ESPRGB', { Text = 'Rainbow Colors', Default = false, Callback = function(v) Settings.ESPRGB = v end })
LeftESPBox:AddSlider('ESPRGBSpeed', { Text = 'Rainbow Speed', Default = 1, Min = 0.1, Max = 5, Rounding = 1, Callback = function(v) Settings.ESPRGBSpeed = v end })

RightHUDBox:AddToggle('HUDEnabledToggle', { Text = 'Enable HUD Info', Default = false, Callback = function(v) hudEnabled = v end })
RightHUDBox:AddToggle('HUDFPSToggle', { Text = 'Display FPS', Default = false, Callback = function(v) hudFPS = v end })
RightHUDBox:AddToggle('HUDSpeedToggle', { Text = 'Display Speed', Default = false, Callback = function(v) hudSpeed = v end })
RightHUDBox:AddToggle('HUDPingToggle', { Text = 'Display Ping', Default = false, Callback = function(v) hudPing = v end })

-- TAB: MISC
local LeftMiscBox = Tabs.Misc:AddLeftGroupbox('World Modification')
local RightMiscBox = Tabs.Misc:AddRightGroupbox('Movement & Camera')

LeftMiscBox:AddToggle('WorldColorToggle', { Text = 'Custom World Color', Default = false, Callback = function(v) worldColorEnabled = v; updateWorldColor() end }):AddColorPicker('WorldColorPicker', {
    Default = Color3.fromRGB(114, 9, 183), Title = 'World Tint', Callback = function(v) customWorldColor = v; if worldColorEnabled then updateWorldColor() end end
})

LeftMiscBox:AddToggle('NightModeToggle', { Text = 'Night Mode', Default = false, Callback = function(v) nightModeEnabled = v end })
LeftMiscBox:AddSlider('NightBrightnessSlider', { Text = 'Night Brightness', Default = 0.5, Min = 0, Max = 2, Rounding = 1, Callback = function(v) nightBrightness = v end })

RightMiscBox:AddToggle('BhopToggle', { Text = 'Bunny Hop', Default = false, Callback = function(v) bhopEnabled = v end })
RightMiscBox:AddToggle('JumpSpeedToggle', { Text = 'Fix Jump Velocity (25)', Default = true, Callback = function(v) jumpSpeedEnabled = v end })
RightMiscBox:AddToggle('SpinToggle', { Text = 'Anti-Aim Spin', Default = false, Callback = function(v) spinEnabled = v; updateSpin() end })
RightMiscBox:AddSlider('SpinSpeedSlider', { Text = 'Spin Speed', Default = 25, Min = 1, Max = 50, Rounding = 0, Suffix = '°', Callback = function(v) SPIN_SPEED = v end })
RightMiscBox:AddToggle('ZoomToggle', { Text = 'Third Person', Default = false, Callback = function(v) zoomEnabled = v; updateZoom() end })
RightMiscBox:AddToggle('AntiAimDownToggle', { Text = 'Pitch Down (Fake Lag Visual)', Default = false, Callback = function(v) Settings.AntiAimDown = v end })
RightMiscBox:AddSlider('ZoomDistanceSlider', { Text = 'Camera Distance', Default = 30, Min = 5, Max = 150, Rounding = 0, Suffix = ' studs', Callback = function(v) DESIRED_ZOOM = v; updateZoom() end })
RightMiscBox:AddSlider('CameraFOVSlider', { Text = 'Camera FOV', Default = 70, Min = 1, Max = 120, Rounding = 0, Suffix = '°', Callback = function(v) cameraFOV = v end })

-- TAB: WALLCAM
local WallCamBox = Tabs.WallCam:AddLeftGroupbox('Spectator Camera')
WallCamBox:AddToggle('WallCamToggle', { Text = 'Enable WallCam', Default = false, Callback = function(v) if v then toggleWallCam() else if wallCamEnabled then toggleWallCam() end end end })
WallCamBox:AddSlider('WallCamDistance', { Text = 'Distance offset', Default = 5, Min = 1, Max = 20, Rounding = 1, Suffix = ' studs', Callback = function(v) CAMERA_DISTANCE = v end })
WallCamBox:AddSlider('WallCamSensitivity', { Text = 'Mouse Sensitivity', Default = 0.5, Min = 0.1, Max = 2, Rounding = 1, Callback = function(v) MOUSE_SENSITIVITY = v end })
...
Library:Notify("neverlose.cc loaded successfully!", 3)