...
local Settings = {
    ...
    AutoTrigger = false,
    VisibleCheck = true,
    PenetrateMaterials = true, -- Настройка простреливаемых текстур
...
-- Проверка на простреливаемые поверхности (стекло, сетки, прозрачность)
local function isPenetrableMaterial(hitPart)
    if not Settings.PenetrateMaterials then return false end
    if not hitPart or not hitPart:IsA("BasePart") then return false end
    
    if hitPart.Material == Enum.Material.Glass or hitPart.Transparency >= 0.5 then
        return true
    end
    
    local nameLower = hitPart.Name:lower()
    if nameLower:find("glass") or nameLower:find("grille") or nameLower:find("fence") or nameLower:find("net") then
        return true
    end
...
local function isVisible(targetPart)
    ...
    local currentOrigin = origin.Position
    ...
    for i = 1, maxIterations do
        ...
        if isPenetrableMaterial(hitPart) then
            ...
            direction = (targetPart.Position - currentOrigin)
        else
            return false
        end
    end
...
CombatTab:CreateToggle({
    Name = "Учитывать прострелы (WallCheck)",
    CurrentValue = true,
    Flag = "PenetrateMaterialsToggle",
    Callback = function(v) Settings.PenetrateMaterials = v end
})
...
Rayfield:Notify({
    Title = "@set9p",
    Content = "Добавлена настройка учета прострелов в меню!",
    Duration = 3,
    Image = 4483362458,
})