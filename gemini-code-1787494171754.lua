-- ============================================
-- SWILL | FULL SCRIPT (WITH WALLCHECK TOGGLE)
-- ============================================
...
local ndsEnabled = false -- НДС (Авто фаер по врагам внутри FOV)
local wallCheckEnabled = true -- Проверка стен для Silent Aim / Фокуса
local aimFOV = 200
local drawFOV = false
local fovCircle = nil
local target = nil
...
-- Функция поиска целей в FOV с учетом настройки проверки стен
local function getClosestPlayerInFOV()
    local closestDistance = math.huge
    local closest = nil
    local center = Camera.ViewportSize / 2

    local function scanPlayers(onlyEnemies)
        for _, player in ipairs(Players:GetPlayers()) do
            if player == LocalPlayer then continue end
            if onlyEnemies and not isEnemy(player) then continue end
            ...
            -- Проверка видимости работает только если включен wallCheckEnabled
            local passesVisibility = not wallCheckEnabled or isVisible(hitPart)

            if passesVisibility then
    ...
    -- 1. Сначала ищем врагов из противоположной команды
    scanPlayers(true)

    -- 2. Если враги не найдены, ищем остальных игроков (кроме себя)
    if not closest then
        scanPlayers(false)
    end
...
-- Постоянное обновление цели каждый кадр
RunService.RenderStepped:Connect(function()
    if dddEnabled or ndsEnabled then 
...
-- ============================
-- FOV КРУГ И НДС (АВТО ФАЕР ПО КРУГУ)
-- ============================
...
-- Имитация клика ровно по центру экрана
local function simulateCenterClick()
    ...
    if now - lastFire >= fireDelay then
        ...
        if VIM then
            ...
            pcall(function()
                VIM:SendMouseButtonEvent(centerPos.X, centerPos.Y, 0, true, game, false)
                task.wait()
                VIM:SendMouseButtonEvent(centerPos.X, centerPos.Y, 0, false, game, false)
            end)
            pcall(function()
                VIM:SendTouchEvent(1, Enum.UserInputState.Begin, centerPos.X, centerPos.Y, game)
                task.wait()
                VIM:SendTouchEvent(1, Enum.UserInputState.End, centerPos.X, centerPos.Y, game)
            end)
        end
    end
end
...
CombatTab:CreateToggle({
    Name = "НДС (Автофаер по врагу в FOV)",
    CurrentValue = false,
    Flag = "NDSToggle",
...
Rayfield:Notify({
    Title = "Готово",
    Content = "Скрипт запущен с переключателем проверки стен!",
    Duration = 3,
    Image = 4483362458,
})