-- ============================================
-- @set9p | SILENT AIM + WALLBANG + AUTO TRIGGER + ESP + BHOP
-- ============================================
...
local Settings = {
    ...
    -- Auto Trigger & Visible Check / Wallbang
    AutoTrigger = false,
    MaxTargetWorldDistance = 150, -- Абсолютный лимит дальности до цели
...
-- ============================
-- ПРОВЕРКА ПРОСТРЕЛИВАЕМОСТИ МАТЕРИАЛОВ (WALLBANG)
-- ============================
local function isWallbangable(part)
    if not part then return false end
    local ignoreMaterials = {
        Enum.Material.Glass,
        Enum.Material.Wood,
        Enum.Material.WoodPlanks,
        Enum.Material.CorrugatedMetal,
        Enum.Material.Ice,
        Enum.Material.Fabric
    }
    
    for _, mat in ipairs(ignoreMaterials) do
        if part.Material == mat then
            return true
        end
    end
    
    if part.Size.X < 1.5 or part.Size.Y < 1.5 or part.Size.Z < 1.5 then
        return true
    end
...
-- ============================
-- ПРОДВИНУТАЯ ПРОВЕРКА ВИДИМОСТИ (С УЧЕТОМ ПРОСТРЕЛА СТЕН)
-- ============================
    ...
    for _, part in ipairs(checkParts) do
        if part and part:IsA("BasePart") then
            ...
            if (origin - targetPos).Magnitude <= Settings.MaxTargetWorldDistance then
                ...
                if onScreen then
                    local canHitThroughWalls = true
                    local currentOrigin = origin
                    local safetyCounter = 0
                    
                    repeat
                        safetyCounter = safetyCounter + 1
                        local result = Workspace:Raycast(currentOrigin, targetPos - currentOrigin, raycastParams)
                        ...
                        if isWallbangable(result.Instance) then
                            table.insert(filterList, result.Instance)
                            raycastParams.FilterDescendantsInstances = filterList
                            currentOrigin = result.Position + (targetPos - currentOrigin).Unit * 0.1
                        else
                            canHitThroughWalls = false
                            break
                        end
                    until safetyCounter > 5
                    
                    if canHitThroughWalls and safetyCounter <= 5 then
                        local finalResult = Workspace:Raycast(currentOrigin, targetPos - currentOrigin, raycastParams)
                        if not finalResult or finalResult.Instance:IsDescendantOf(model) then
                            return true
                        end
                    end
                end
...
CombatTab:CreateToggle({
    Name = CanHook and "Silent Aim (Wallbang)" or "Silent Aim [NO WORK]",
    CurrentValue = false,
...
Rayfield:Notify({
    Title = "@set9p",
    Content = "Скрипт запущен: Wallbang (прострел стен) активен!",
    Duration = 3,
    Image = 4483362458,
})